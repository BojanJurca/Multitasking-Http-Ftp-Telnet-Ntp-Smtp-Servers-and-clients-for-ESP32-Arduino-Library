...
void httpServer_t::webSocket_t::__runConnectionTask__ () {
    ...
    do { // while keepAlive
        ...
        if (__httpRequestHandlerCallback__) {
            ...
            if (!tmpContent) { // out of memory
                cout << ( dmesgQueue << "[httpConn] " "out of memory" );
                __transport__->sendString (reply503);
                return;
            }
            /// httpReplyContent = tmpContent;
            ...
            #endif
                memcpy (tmp, &tmpContent, sizeof (String));
                memcpy (&tmpContent, &httpReplyContent, sizeof (String));
                memcpy (&httpReplyContent, tmp, sizeof (String));
            #if defined(__GNUC__) && __GNUC__ >= 8
