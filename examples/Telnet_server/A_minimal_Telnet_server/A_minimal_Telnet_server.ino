...
#define HOSTNAME "Esp32Server"    // Choose your server's name - this is how Telnet server will introduce itself to its clients


// 1️⃣ Choose which built-in Telnet commands will be included prior to #include <telnetServer.h>
...
void setup () {
  ...
  // 3️⃣ Create static (so it would contiune to run even when setup finishes) Telnet server instance
                                      // Optional arguments (when thread-safe file system is not included):
  static telnetServer_t telnetServer; // Cstring<255> (*getUserHomeDirectory) (const Cstring<64>& userName, const Cstring<64>& password) = NULL
  ...
  // 4️⃣ Check if Telnet server is running
  if (telnetServer)
  ...
  // 6️⃣ Setting the time is only important for time related commands
