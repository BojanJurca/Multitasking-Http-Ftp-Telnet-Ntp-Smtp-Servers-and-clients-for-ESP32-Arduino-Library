...
// 2️⃣ Choose which built-in Telnet commands will be included prior to #include <telnetServer.h>
...
void setup () {
  ...
  // 5️⃣ Create static (so it would contiune to run even when setup finishes) Telnet server instance that would use thread-safe wrapper tsfs arround LittleFS (or FFat or SD)
									                    // Optional arguments (when thread-safe file system is included):
  static telnetServer_t telnetServer; // threadSafeFS::FS& fileSystem 
  ...
  // 6️⃣ Check if Telnet server is running
  if (telnetServer)
  ...
  // 7️⃣ Use Telent client to connect to ESP32's IP address
  ...
  // 8️⃣ Setting the time is only important for time related commands
