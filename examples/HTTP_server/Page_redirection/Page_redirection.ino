...
String httpRequestHandlerCallback (const char *httpRequest, httpServer_t::httpConnection_t *hcn) {
  ...
  if (httpRequestIs ("GET /maintenance.html "))
      return  "<!DOCTYPE html>\n"
              "<html lang='en'>\n"
              "   <head>\n"
              "      <meta charset='UTF-8'>\n"
              ...
              "   </head>\n"
              "   <body>\n"
              "      <h1>The service is not available due to maintenance.</h1>\n"
              "   </body>\n"
              "</html>";
  ...
  if (maintenance) {
    ...
    // 1️⃣ Redirect browser to maintenance.html
    hcn->setHttpReplyHeaderField ("Location", "/maintenance.html");
